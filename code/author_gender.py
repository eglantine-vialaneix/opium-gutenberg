...
_HONOR = re.compile(r"\b(Mrs?\.?|Sir|Lord|Lady|Baron|Baroness)\s*")
...
_DUAL = (
    ...
    r"British-American", r"Canadian-American", r"Australian-American",
)
...
def _parse_gutenberg_name(raw: str) -> tuple:
    """
    ...
    if match:
        ...
        first = first.replace(honorific, '').strip()
        
    first_p = re.search(_PAREN, first)
    first = first_p.group(1).strip() if first_p else first
            
    last, first = re.sub(r"\s{2,}", " ", last), re.sub(r"\s{2,}", " ", first)
 
    norm_name = last + ' ' + first
...
def _scrape_wikipedia(name: str, h_gender=None) -> AuthorRecord | None:
    """
    ...
    # ── nationality ────────────────────────────────────────────────────────
    nationalities = []
    for m in _NAT_PATTERN.finditer(extract):
        nat = m.group(1).title()
    ...
    if h_gender:
        ...
        text_lower = (desc + " " + extract).lower().split(' ')
        female_signals = ("she ", "her ", " woman ", "female", "mrs")
        male_signals   = ("he ",  "his ", "him", " man ", "male", "sir")
        female_signals_count = sum([s in female_signals for s in text_lower])
        male_signals_count = sum([s in male_signals for s in text_lower])
        if female_signals_count > male_signals_count:
            gender = "female"
        elif female_signals_count == male_signals_count:
            gender = f"ambiguous (M={male_signals_count} ; F={female_signals_count})"
            print(text_lower)
        else:
            gender = "male"
...
#             if any(s in bio for s in ("she ", "her ", "woman ", "female", "mrs", "miss", "lady")):
#                 gender = "female"
#             elif any(s in bio for s in ("he ", "his ", "man ", "male", "mr", "sir")):
#                 gender = "male"
...
def scrape_author(raw_name: str) -> AuthorRecord:
    ...
    """
    normalised_name, h_gender = _parse_gutenberg_name(raw_name)

    combined: AuthorRecord | None = None

    for source_label, scraper in _SCRAPERS:
        try:
            result = scraper(normalised_name)
        except Exception as exc:
